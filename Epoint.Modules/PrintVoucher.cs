...
namespace Epoint.Modules
{
	public class PrintVoucher
	{
        public static bool Print(DataRow drPhView, bool bPreview, bool bShowDialog, ref bool bInVisibleNextPrint, ref string strReport_File_First)
		{
			...
			string strMa_Tte = (string)drPhView["Ma_Tte"];
            ...
            dtHeader.Columns.Add("DOC_TIEN2", typeof(string));//Đọc tiền giá vốn nếu Hóa đơn kiêm phiếu xuất
            ...
            if (Element.sysLanguage == enuLanguageType.Vietnamese)
            {
                ...
                dtHeader.Rows[0]["Doc_Tien2"] = !bIs_Vnd ? Common.ReadMoney(Convert.ToDouble(drHeader["TTien_Von_Nt"]), strMa_Tte) : Common.ReadMoney(Convert.ToDouble(drHeader["TTien_Von"]), Element.sysMa_Tte.ToString());
            }
            else
            {
                ...
                dtHeader.Rows[0]["Doc_Tien2"] = !bIs_Vnd ? Common.ReadMoneyE(Convert.ToDouble(drHeader["TTien_Von_Nt"]), strMa_Tte) : Common.ReadMoneyE(Convert.ToDouble(drHeader["TTien_Von"]), Element.sysMa_Tte.ToString());
            }
