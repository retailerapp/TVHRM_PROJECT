...
namespace Epoint.Modules
{
    public class Voucher
    {
        ...
        public static bool SQLDeleteCt(string strStt, string strMa_Ct)
        {
            ...
            {
                string strCreate_User = (string)SQLExec.ExecuteReturnValue("SELECT ISNULL(MAX(Create_Log), '') FROM GLVOUCHER WHERE Stt = '" + strStt + "'");
        ...
        public static void Calc_Chiet_Khau(DataRow drEditCt)
        {
            ...
            double dbChiet_Khau = (drEditCt["Chiet_Khau"] == DBNull.Value) ? 0 : Convert.ToDouble(drEditCt["Chiet_Khau"]);
            double dbChiet_Khau_Org = (drEditCt["Chiet_Khau", DataRowVersion.Original] == DBNull.Value) ? 0 : Convert.ToDouble(drEditCt["Chiet_Khau", DataRowVersion.Original]);
            ...
            //dbTien2 = Convert.ToDouble(drEditCt["Tien_Nt9"]);
            //dbTien_Nt2 = Math.Round(Convert.ToDouble(drEditCt["Tien_Nt9"]) * dbTy_Gia, 2, MidpointRounding.AwayFromZero);
        ...
        public static void FormatTien_Nt(dgvControl dgv, string strMa_Tte)
        {
            ...
            if (dgv.Columns.Contains("Gia_Nt9"))
                dgv.Columns["Gia_Nt9"].DefaultCellStyle.Format = strMa_Tte == Element.sysMa_Tte ? "N2" : "N2";
        ...
        public static string GetDuCuoi(DataRow drEditCt, string strTk)
        {
            ...
            if (Convert.ToString(dr["Ten_Tk"]) != "")
                return dr["Ten_Tk"].ToString();
            else
                return "";
        }
        ...
        public static void UpdateSo_Ct(frmVoucher_Edit frmEditCt)
        {
            if (frmEditCt.enuNew_Edit == enuEdit.New || frmEditCt.enuNew_Edit == enuEdit.Copy)
            {
                ...
                {
                    string strMsg = Element.sysLanguage == enuLanguageType.Vietnamese ? "Chứng từ số: " + strSo_Ct + " Ngày: " + dNgay_Ct.ToShortDateString() +
                     " đã tồn tại.\n Bạn có muốn tự tăng số không ?" : "Voucher No.: " + strSo_Ct + " Date: " + dNgay_Ct.ToShortDateString() + " already exists.\n Would you like to increase the number of voucher ?";
        ...
        public static bool CheckDuplicateInvoice(frmVoucher_Edit frm)
        {
            ...
            foreach (DataRow dr in frm.dtEditCt.Rows)
            {
                ...
                string strQuery = "SELECT COUNT(So_Ct0) FROM vw_THUEVAT WHERE So_Ct0 = '" + (string)dr["So_Ct0"] + "' AND So_Seri0 = '" +
                    (string)dr["So_Seri0"] + "' AND Stt <> '" + (string)dr["Stt"] + "' AND Ma_Ct = '" + (string)dr["Ma_Ct"] +
                    "' AND Ma_So_Thue = '" + (string)dr["Ma_So_Thue"] + "'";

                int obj = (int)SQLExec.ExecuteReturnValue(strQuery);
                if (obj >= 1)
                {
                    string strMsg = Element.sysLanguage == enuLanguageType.Vietnamese ? "Số hóa đơn  = {" + (string)dr["So_Ct0"] + "} và số Seri = {" + (string)dr["So_Seri0"] +
                        "} đã tồn tại, bạn có muốn lưu không?" : "Bill No.  = {" + (string)dr["So_Ct0"] + "} and Seri No. = {" + (string)dr["So_Seri0"] + "} already exists, do you want to save ?";
        ...
        {
            string strMsg = Element.sysLanguage == enuLanguageType.Vietnamese ? "Bạn muốn import dữ liệu hay kết xuất ?" : "Do you want to import data or output ?";
        ...
        public static DataTable GetTVPValue(string strTableName, string strTableTypeName, DataTable dtTableSource)
        {
            ...
            DataTable table = DataTool.SQLGetDataTable(strTableName, strColumnLst, "0=1", "");
