...
namespace BarcodeLib
{
    ...
    public class Barcode : IDisposable
    {
        ...
        private string sTen1 = "";
        private string sTen2 = "";
        ...
        /// <param name="Height">Height of the resulting barcode.(pixels)</param>
        /// <returns>Image representing the barcode.</returns>
        public Image Encode(TYPE iType, string StringToEncode, string Ten1, string Ten2, Color ForeColor, Color BackColor, int Width, int Height)
        {
            this.Width = Width;
            this.Height = Height;
            this.sTen1 = Ten1;
                            ...
                            using (Pen pen = new Pen(ForeColor, iBarWidth))
                            {
                                ...
                                while (pos < Encoded_Value.Length)
                                {
                                    ...
                                    if (Encoded_Value[pos] == '1')
                                        g.DrawLine(pen, new Point((pos * iBarWidth) + shiftAdjustment + bearerwidth + iquietzone, 0), new Point((pos * iBarWidth) + shiftAdjustment + bearerwidth + iquietzone, Height + istar));
                                ...
                                g.DrawLine(pen, new Point(0, istar), new Point(b.Width, 0));//top
                                g.DrawLine(pen, new Point(0, b.Height + istar), new Point(b.Width, b.Height + istar));//bottom
                                g.DrawLine(pen, new Point(0, istar), new Point(0, b.Height + istar));//left
                                g.DrawLine(pen, new Point(b.Width, istar), new Point(b.Width, b.Height + istar));//right
                            }//using
                        }//using
                            ...
                            using (Pen backpen = new Pen(BackColor, iBarWidth / iBarWidthModifier))
                            {
                                using (Pen pen = new Pen(ForeColor, iBarWidth / iBarWidthModifier))
                                {
                                    while (pos < Encoded_Value.Length)
                                    {
                                        ...
                                        if (Encoded_Value[pos] == '1')
                                            g.DrawLine(pen, new Point(pos * iBarWidth + shiftAdjustment + (int)(iBarWidth * 0.5), 12), new Point(pos * iBarWidth + shiftAdjustment + (int)(iBarWidth * 0.5), Height-40));
        ...
        private Image Label_Generic(Image img)
        {
            try
            {
                ...
                using (Graphics g = Graphics.FromImage(img))
                {
                    Font fontCode = new Font("Microsoft Sans Serif", 7, FontStyle.Regular);
                    Font fontTitle = new Font("Microsoft Sans Serif", 8, FontStyle.Regular);
                    StringFormat f = new StringFormat();
                    ...
                    g.FillRectangle(new SolidBrush(this.BackColor), new RectangleF((float)0, (float)0, (float)0, (float)fontTitle.Height));
                    g.DrawString(this.sTen1, fontTitle, new SolidBrush(this.ForeColor), new RectangleF((float)0, (float)0, (float)img.Width, (float)0), f);

                    //color a background color box at the bottom of the barcode to hold the string of data
                    g.FillRectangle(new SolidBrush(this.BackColor), new RectangleF((float)0, (float)LabelY -25, (float)img.Width, (float)font.Height));
                    //draw datastring under the barcode image
                    g.DrawString(this.RawData, fontCode, new SolidBrush(this.ForeColor), new RectangleF((float)0, (float)LabelY - 25, (float)img.Width, (float)font.Height), f);

                    g.FillRectangle(new SolidBrush(this.BackColor), new RectangleF((float)0, (float)LabelY -15, (float)img.Width, (float)font.Height));
                    //draw datastring under the barcode image
                    g.DrawString(this.sTen2, fontTitle, new SolidBrush(this.ForeColor), new RectangleF((float)0, (float)LabelY-15, (float)img.Width, (float)font.Height + 20), f);
